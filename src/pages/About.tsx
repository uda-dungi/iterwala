    ...
    <div>
      ...
      <section className="relative h-[50vh] sm:h-[60vh] md:h-[70vh] flex items-center justify-center text-center overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}
          ...
          <h1 className="font-display text-4xl sm:text-5xl md:text-8xl text-ivory mt-4 leading-[0.95]">
            A Tradition <span className="italic text-gold">Bottled</span>
          </h1>
          <p className="text-base sm:text-lg text-muted-foreground mt-4 sm:mt-6 font-serif italic">
      ...
      <section className="container py-10 md:py-16 text-center">
        ...
        <h2 className="font-display text-3xl sm:text-4xl md:text-6xl text-ivory mt-3">Legacy of 25 Years</h2>
        <p className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto mt-4 sm:mt-6 leading-relaxed">
        ...
        <div className="flex items-center justify-center gap-6 sm:gap-12 mt-6 sm:mt-8">
          <div>
      ...
      {/* Story */}
      <section className="container py-10 md:py-20 grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
          transition={{ duration: 0.8 }} className="space-y-4 sm:space-y-6">
          <p className="text-[10px] tracking-[0.5em] uppercase text-primary">Kannauj · India</p>
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-ivory">The Itr Capital of the World</h2>
      ...
      <section className="container py-10 md:py-20">
        <div className="text-center mb-8 md:mb-12">
          ...
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-ivory mt-2">Our Tenets</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-4 md:gap-6">
          {[
            ...
            <div key={v.n} className="luxury-card p-6 sm:p-10">
              <span className="font-display text-4xl sm:text-6xl text-gold/60">{v.n}</span>
              <h3 className="font-serif text-xl sm:text-2xl text-ivory mt-3 sm:mt-4">{v.t}</h3>
              <p className="text-sm sm:text-base text-muted-foreground mt-2 sm:mt-3 leading-relaxed">{v.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="container py-10 md:py-20">
        <div className="text-center mb-8 md:mb-12">
          ...
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-ivory mt-2">From Petal to Bottle</h2>
        </div>
        <div className="space-y-4 max-w-3xl mx-auto">
          {[
            ...
            <motion.div key={i}
              ...
              className="flex gap-4 sm:gap-6 items-start luxury-card p-4 sm:p-6">
              <span className="font-display text-2xl sm:text-3xl text-gold w-10 sm:w-12 shrink-0">0{i + 1}</span>
              <div>
                <h3 className="font-serif text-lg sm:text-xl text-ivory">{s.t}</h3>
                <p className="text-sm sm:text-base text-muted-foreground mt-1">{s.d}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <section className="container py-14 md:py-24 text-center">
        <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-ivory">Begin your Itrawala journey.</h2>
        <p className="text-sm sm:text-base text-muted-foreground mt-4 max-w-xl mx-auto">Explore the full collection — or take the signature scent quiz.</p>
