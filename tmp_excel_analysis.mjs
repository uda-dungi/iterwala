  ...
  for (let j = i + 1; j < parentEntries.length; j++) {
    ...
    if (sim >= 0.5 && sim < 1) {
      ...
      if (priceRatio > 0.7 || (Math.abs(avgPriceA - avgPriceB) < 200 && sim > 0.65)) {
        manualReview.push({ a: a.key, b: b.key, similarity: Number(sim.toFixed(2)), avgPriceA: Number(avgPriceA.toFixed(2)), avgPriceB: Number(avgPriceB.toFixed(2)), priceRatio: Number(priceRatio.toFixed(2)), namesA: [...a.names].slice(0,3), namesB: [...b.names].slice(0,3) });
      }
    }
  }
}
...
console.log(JSON.stringify({
  ...
  parentProducts: parentProductReport.slice(0, 200),
  ...
  manualReviewCandidates: manualReview.slice(0, 200),
